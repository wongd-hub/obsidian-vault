...
We can use Analysis of Variance (ANOVA) to analyse 3+ means (whereas the t-distribution only works for comparing two means).
...
As with previous hypothesis tests:
...
i.e. homoskedastic groups. Especially important if sample sizes vary between groups. Can check this using side-by-side boxplots and by calculating standard deviation within each group.
